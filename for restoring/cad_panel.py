...
class CADPanel(QWidget):
    ...
    def _build(self):
        ...
        kl = QVBoxLayout(kc); kl.setContentsMargins(22,16,22,16); kl.setSpacing(8)
        kh = QHBoxLayout()
        kt = QLabel("Anthropic API Key"); kt.setFont(QFont("Segoe UI",12,QFont.Weight.Bold))
        kh.addWidget(kt); kh.addStretch()
        self.key_status = QLabel("● Not set — OpenCV mode")
        self.key_status.setStyleSheet(f"color:{COLORS['warning']};font-size:12px;")
        kh.addWidget(self.key_status); kl.addLayout(kh)
        kd = QLabel("With key: Claude Vision identifies ALL shapes, text, dims, patterns (recommended).\n"
                    "Without key: Smart OpenCV contour tracing (shapes only, no text recognition).")
        kd.setStyleSheet(f"color:{COLORS['text_secondary']};font-size:12px;"); kd.setWordWrap(True)
        kl.addWidget(kd)
        ...
        if saved: self.key_input.setText(saved); self._upd_key_status(saved)
        self.key_input.textChanged.connect(lambda t: self._upd_key_status(t.strip()))
        kr.addWidget(self.key_input)
        tb = QPushButton("Show"); tb.setObjectName("secondaryBtn"); tb.setFixedWidth(58)
        tb.clicked.connect(lambda: self._toggle_vis(tb)); kr.addWidget(tb)
        sb = QPushButton("Save"); sb.setObjectName("primaryBtn"); sb.setFixedWidth(72)
        sb.clicked.connect(self._save_key); kr.addWidget(sb)
        kl.addLayout(kr); layout.addWidget(kc); layout.addSpacing(14)
    ...
    def _upd_key_status(self, key):
        if key and key.startswith("sk-ant"):
            self.key_status.setText("● API key set — Claude Vision active")
            self.key_status.setStyleSheet(f"color:{COLORS['success']};font-size:12px;")
        elif key:
            self.key_status.setText("● Key entered (will validate on use)")
            self.key_status.setStyleSheet(f"color:{COLORS['warning']};font-size:12px;")
        else:
            self.key_status.setText("● Not set — OpenCV mode")
            self.key_status.setStyleSheet(f"color:{COLORS['warning']};font-size:12px;")
