...
class SidebarButton(QPushButton):
    def __init__(self, text: str, code: str = "", obj_name: str = "sidebarButton", parent=None):
        super().__init__(parent)
        self._label = text
        self._code = code
        self.setText(f"{code}  {text}" if code else text)
...
class MainWindow(QMainWindow):
    ...
    def _build_sidebar(self) -> QFrame:
        ...
        workflow = QLabel("WORKFLOWS")
        workflow.setObjectName("navLabel")
        layout.addWidget(workflow)

        self.btn_cd = SidebarButton("CD Processing", "01")
        self.btn_cad = SidebarButton("CAD Process", "02")
        self.btn_cad2 = SidebarButton("CAD Process 2", "03")
        self.btn_gad = SidebarButton("GAD Checking", "04")
        self.btn_hydro = SidebarButton("Hydraulic Calcs", "05")
        ...
        for btn in (self.btn_cd, self.btn_cad, self.btn_cad2, self.btn_gad, self.btn_hydro):
            layout.addWidget(btn)

        layout.addSpacing(12)
        reference = QLabel("REFERENCE")
        reference.setObjectName("navLabel")
        layout.addWidget(reference)

        self.btn_kb = SidebarButton("Knowledge Base", "06")
        ...
        self.btn_settings = SidebarButton("Settings", "07", obj_name="settingsBtn")
        self.btn_settings.clicked.connect(lambda: self._switch(6))
        layout.addWidget(self.btn_settings)

        version = QLabel("v2.0 local build")
