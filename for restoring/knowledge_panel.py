...
class KnowledgePanel(QWidget):
    ...
    def _build(self):
        ...
        # ── API Keys settings bar ─────────────────────────────────────────
        outer.addWidget(self._build_api_settings_bar())
    ...
    # ── API Keys Settings Bar ─────────────────────────────────────────────────
    def _build_api_settings_bar(self):
        """
        Compact collapsible bar for entering Gemini + Claude API keys.
        Gemini is tried first; Claude is the automatic fallback.
        Keys are persisted via QSettings.
        """
        bar = QFrame()
        bar.setObjectName("card")
        bar.setStyleSheet(
            f"QFrame#card{{background:{COLORS['navy']};border:none;"
            f"border-bottom:1px solid {COLORS['border']};border-radius:0px;padding:0px;}}"
        )
        ...
        toggle_row.setContentsMargins(0, 8, 0, 8)

        key_icon = QLabel("🔑  API Keys")
        key_icon.setFont(QFont("Segoe UI", 10, QFont.Weight.Bold))
        key_icon.setStyleSheet(f"color:{COLORS['accent']};")
        toggle_row.addWidget(key_icon)

        # Status indicator — shows which provider is active
        self._key_status_lbl = QLabel("")
        self._key_status_lbl.setFont(QFont("Segoe UI", 9))
        self._key_status_lbl.setStyleSheet(f"color:{COLORS['text_sidebar']};")
        toggle_row.addWidget(self._key_status_lbl)
        toggle_row.addStretch()

        self._settings_toggle_btn = QPushButton("▼  Configure")
        self._settings_toggle_btn.setFixedHeight(26)
        self._settings_toggle_btn.setStyleSheet(
        ...
        self._settings_toggle_btn.clicked.connect(self._toggle_api_settings)
        toggle_row.addWidget(self._settings_toggle_btn)
        bar_lay.addLayout(toggle_row)

        # ── Expandable key fields (hidden by default) ─────────────────────
        self._api_settings_body = QFrame()
        self._api_settings_body.setVisible(False)
        body_lay = QVBoxLayout(self._api_settings_body)
        body_lay.setContentsMargins(0, 4, 0, 10)
        body_lay.setSpacing(6)

        # Explanation label
        info = QLabel(
            "Gemini is used first. If Gemini fails (quota / error), Claude is used automatically. "
            "You can set one or both keys."
        )
        info.setWordWrap(True)
        info.setStyleSheet(
            f"color:{COLORS['text_sidebar']};font-size:10px;"
            f"background:{COLORS['info_bg']};border-left:3px solid {COLORS['info']};"
        ...
        # Gemini key field
        g_col = QVBoxLayout(); g_col.setSpacing(3)
        g_lbl = QLabel("Google Gemini API Key  (AIza…)"); g_lbl.setObjectName("fieldLabel")
        g_lbl.setStyleSheet(f"color:{COLORS['text_sidebar']};font-size:10px;font-weight:600;")
        g_col.addWidget(g_lbl)
        self._gemini_key_input = QLineEdit()
        self._gemini_key_input.setEchoMode(QLineEdit.EchoMode.Password)
        self._gemini_key_input.setPlaceholderText("AIza…  (Google AI Studio key)")
        self._gemini_key_input.setMinimumHeight(32)
        self._gemini_key_input.setText(self._gemini_key())
        self._gemini_key_input.setStyleSheet(
            f"QLineEdit{{background:{COLORS['input_bg']};border:1px solid {COLORS['border_dark']};"
            f"border-radius:6px;padding:4px 10px;font-size:11px;color:{COLORS['text_primary']};}}"
            f"QLineEdit:focus{{border-color:{COLORS['accent']};}}"
        )
        g_col.addWidget(self._gemini_key_input)
        fields_row.addLayout(g_col, 1)

        # Claude key field
        c_col = QVBoxLayout(); c_col.setSpacing(3)
        c_lbl = QLabel("Anthropic Claude API Key  (sk-ant…)"); c_lbl.setObjectName("fieldLabel")
        c_lbl.setStyleSheet(f"color:{COLORS['text_sidebar']};font-size:10px;font-weight:600;")
        c_col.addWidget(c_lbl)
        self._claude_key_input = QLineEdit()
        self._claude_key_input.setEchoMode(QLineEdit.EchoMode.Password)
        self._claude_key_input.setPlaceholderText("sk-ant-…  (Anthropic Console key)")
        self._claude_key_input.setMinimumHeight(32)
        self._claude_key_input.setText(self._claude_key())
        self._claude_key_input.setStyleSheet(
            f"QLineEdit{{background:{COLORS['input_bg']};border:1px solid {COLORS['border_dark']};"
            f"border-radius:6px;padding:4px 10px;font-size:11px;color:{COLORS['text_primary']};}}"
            f"QLineEdit:focus{{border-color:{COLORS['accent']};}}"
        )
        c_col.addWidget(self._claude_key_input)
        fields_row.addLayout(c_col, 1)

        # Save button
        btn_col = QVBoxLayout(); btn_col.setSpacing(3)
        btn_col.addWidget(QLabel(""))   # spacer to align with fields
        ...
        body_lay.addLayout(fields_row)
        bar_lay.addWidget(self._api_settings_body)

        # Populate status on startup
        self._refresh_key_status()
        return bar

    def _toggle_api_settings(self):
        visible = self._api_settings_body.isVisible()
        self._api_settings_body.setVisible(not visible)
        self._settings_toggle_btn.setText(
            "▲  Close" if not visible else "▼  Configure"
        )

    def _save_api_keys(self):
        gk = self._gemini_key_input.text().strip()
        ck = self._claude_key_input.text().strip()
        self.settings.setValue("gemini_api_key", gk)
        self.settings.setValue("claude_api_key",  ck)
        # Clear legacy key to avoid confusion
        self.settings.remove("api_key")
        self._refresh_key_status()
        self._toggle_api_settings()   # collapse after saving

    def _refresh_key_status(self):
        gk = self._gemini_key()
        ck = self._claude_key()
        if gk and ck:
            msg  = "✅  Gemini (primary)  +  Claude (fallback)  active"
            color = COLORS.get("success", "#00C8A0")
        elif gk:
            msg  = "🟡  Gemini key only — no Claude fallback"
            color = COLORS.get("warning", "#F5A623")
        elif ck:
            msg  = "🟡  Claude key only — Gemini not configured"
            color = COLORS.get("warning", "#F5A623")
        else:
            msg  = "⚠️  No API keys set — quiz will use cached questions only"
            color = COLORS.get("error", "#E05C5C")
        self._key_status_lbl.setText(msg)
        self._key_status_lbl.setStyleSheet(f"color:{color};font-size:9px;")
    ...
    def _build_quiz_section(self):
        ...
        # ── Title row ─────────────────────────────────────────────────────
        row0 = QHBoxLayout()
        title = QLabel("🎯  Quiz App — PYQ-Based Practice")
        ...
        # Header row: Q number | difficulty | timer | Check Needed btn
